                    ...
                    std::string password )
{
  m_ssid = ssid;
  m_password = password;
  ESP_LOGI( LOG_TAG, "Using SSID %s and PASSWORD %s", m_ssid.c_str(), m_password.c_str() );
  wifi_task_queue_item item = {
...
void wifi::connect( void ) {
    ...
    ESP_LOGI( LOG_TAG, "start wifi connect thread" );

    esp_err_t ret = nvs_flash_init();
    if ( ret == ESP_ERR_NVS_NO_FREE_PAGES ||
        ret == ESP_ERR_NVS_NEW_VERSION_FOUND )
    {
      ESP_LOGI( LOG_TAG, "Erasing NVS mem" );
      ESP_ERROR_CHECK( nvs_flash_erase() );
      ret = nvs_flash_init();
    }  

    ESP_LOGI( LOG_TAG, "Calling esp_wifi_init" );

    wifi_init_config_t config = WIFI_INIT_CONFIG_DEFAULT();
    esp_wifi_init( &config );

    ESP_LOGI( LOG_TAG, "Calling esp_wifi_set_mode" );
    esp_wifi_set_mode( WIFI_MODE_STA );

    ESP_LOGI( LOG_TAG, "Calling esp_netif_init" );
    esp_netif_init();

    ESP_LOGI( LOG_TAG, "Calling esp_event_loop_create_default" );
    esp_event_loop_create_default();

    ESP_LOGI( LOG_TAG, "Calling esp_netif_create_default_wifi_sta" );
    ...
    ESP_LOGI( LOG_TAG, "Register event handler for any ID" );
    ...
    ESP_LOGI( LOG_TAG, "Register event handler for IP event" );
    ...
    ESP_LOGI( LOG_TAG, "Fetch SSID and password" );
    ...
    ESP_LOGI( LOG_TAG, "ssid: %s passwd: %s", wifi_cfg.sta.ssid, wifi_cfg.sta.password );
    ...
    ESP_LOGI( LOG_TAG, "Calling esp_wifi_set_config" );
    esp_wifi_set_config( WIFI_IF_STA, &wifi_cfg );

    ESP_LOGI( LOG_TAG, "Calling esp_wifi_start" );
    esp_wifi_start();

    ESP_LOGI( LOG_TAG, "Calling xEventGroupWaitBits" );
